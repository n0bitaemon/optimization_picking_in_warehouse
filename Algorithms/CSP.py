...
def warehouse_picking_routing(d,Q,q):
    ...
    #1. The picker must start at the depot (shelf 0)
    # c = 0
    model.Add(p[0] == 1) #Add visit 0 -> p[0] = 1

    # for shelfi in range(1,M+1):
    #     c+=x[0][shelfi]
    # model.Add(c == 1)

    #The picker must end at the depot(shelf 0)
    # c = 0
    # for shelfj in range(1,M+1): #range(2, M+1) -> (1, M+1)
    #     c+=x[shelfj][0]
    # model.Add(c == 1)         
    ...
    #4. When picker comes to shelf j, all goods any type the picker need at this shelf is picked up
    c = 0
...
if __name__ == "__main__":
    ...
    print('The time taken is: ',time.time())
